var PinyinMatch = (function() {
  ...
  function initDB() {
    ...
    } else if (db.isOpen && !db.isOpen()) {
      console.warn('[DB] db 已关闭，准备重新打开');
    }
    return db;
  }
  ...
  async function matchProductsFromRawList(rawProducts) {
    ...
    for (var i = 0; i < rawProducts.length; i++) {
      ...
      if (nameIndex[exactKey] && nameIndex[exactKey].length > 0) {
        ...
        if (candidates.length === 1) {
          ...
          if (modelMatches.length === 1) {
            ...
            if (specMatch) {
              matchedProduct = { id: specMatch.id, name: specMatch.name, code: 'PRD-' + specMatch.id, sku: specMatch.model || '-', spec: specMatch.spec || '', price: specMatch.price || '' };
              ...
              var cp = modelMatches[0];
              matchedProduct = { id: cp.id, name: cp.name, code: 'PRD-' + cp.id, sku: cp.model || '-', spec: cp.spec || '', price: cp.price || '' };
              matchId = cp.id;
              matchType = 'name';
              status = 'pending';
              exactCount++;
      ...
      if (!matchId) {
        ...
        if (matchCandidates.length > 0) {
          ...
          if (matchId) {
            ...
            if (mp) {
              matchedProduct = { id: mp.id, name: mp.name, code: 'PRD-' + mp.id, sku: mp.model || '-', spec: mp.spec || '', price: mp.price || '' };
            }
            fuzzyCount++;
          }
        }
      ...
      results.push({
        ...
        matchedPrice: matchedProduct ? matchedProduct.price : '',
        matchType: matchType,
        status: status,
