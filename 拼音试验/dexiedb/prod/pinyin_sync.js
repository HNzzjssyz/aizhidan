...
(function (global) {
  ...
  var DIFF_RATE_THRESHOLD = 0.2;
  var ID_FETCH_PAGE_SIZE = 5000;
  var MIN_IDS_FOR_DIFF_CHECK = 1000;
  ...
  async function fetchFullPaged(dtname, field, batchSize, hooks) {
    ...
    var fetchTotal = Math.min(total, batchSize * 10);
    var list = [];
  ...
  async function incrementalSync(decision, hooks) {
    ...
    if (lastTime) {
      ...
      var pList = await fetchModifiedByTime('product', 'id,pro_name,model,spec,price,modstm',
  ...
  async function fetchFullData(hooks) {
    ...
    for (var pi = 0; pi * PRODUCT_BATCH_SIZE < productTotal; pi++) {
      ...
      var params2 = {
        ...
        field: 'id,pro_name,model,spec,price', flag: 1,
